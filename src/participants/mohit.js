import mohitImg from "./avatars/img/mohit.jpg";

const mohit = {
  id: "5",
  name: "Mohit Rana",
  university: "Graphic Era Hill University",
  img: mohitImg,
  pr: [50, 114, 183, 69, 128, 691],
  prLink: [
    'https://github.com/iamAyushChamoli/hacktoberfest-24/pull/50',
    'https://github.com/aditya-bhaumik/Pathsphere/pull/114',
    'https://github.com/GarimaSingh0109/Resum-Resume/pull/183',
    'https://github.com/ML-Fusion-Lab/ML-Fusion-Lab-Website/pull/69',
    'https://github.com/RamakrushnaBiswal/PlayCafe/pull/128',
    'https://github.com/aditya-bhaumik/Pathsphere/pull/691'
  ]
};

export default mohit;
